...
def create_signal_default(n_points=128, fs=200., high_fq=50., low_fq=3.,
                          low_fq_width=1., noise_level=0.1,
                          random_state=42, *args, **kwargs):
    """Call create_signal with default values, used for testing only."""
    return create_signal(n_points, fs, high_fq, low_fq, low_fq_width,
...
def test_correct_size():
    ...
    for n_points_in in np.int_(np.logspace(1, 3, 5)):
        signal = create_signal_default(n_points=n_points_in)
...
def test_random_state():
    ...
    sig_0 = create_signal_default(random_state=0)
    sig_1 = create_signal_default(random_state=1)
    sig_2 = create_signal_default(random_state=0)
...
def test_amplitude():
    ...
    sig_0 = create_signal_default(high_fq_amp=0, low_fq_amp=0, noise_level=0)
    zeros = np.zeros_like(sig_0)
    assert_array_equal(sig_0, zeros)

    sig_1 = create_signal_default(high_fq_amp=1, low_fq_amp=0, noise_level=0)
    sig_2 = create_signal_default(high_fq_amp=2, low_fq_amp=0, noise_level=0)
    ...
    sig_1 = create_signal_default(high_fq_amp=0, low_fq_amp=0, noise_level=1)
    sig_2 = create_signal_default(high_fq_amp=0, low_fq_amp=0, noise_level=2)
    assert_almost_equal(sig_1.std(), 1, decimal=1)
    assert_almost_equal(sig_2.std(), 2, decimal=1)

    sig_1 = create_signal_default(high_fq_amp=0, low_fq_amp=1, noise_level=0)
    sig_2 = create_signal_default(high_fq_amp=0, low_fq_amp=2, noise_level=0)
